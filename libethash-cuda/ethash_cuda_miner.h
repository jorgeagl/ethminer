...
public:
	struct search_hook
	{
		...
		virtual void found(uint64_t const* nonces) = 0;
		virtual void searched(uint64_t start_nonce, uint32_t count) = 0;
	};
...
public:
	...
	bool init(size_t numDevices, ethash_light_t _light, uint8_t const* _lightData, uint64_t _lightSize, unsigned _deviceId, bool _cpyToHost, uint8_t * &hostDAG);
