...
bool CUDAMiner::init(const h256& seed)
{
	...
	try {
		...
		innerInit(getNumDevices(), light->light, lightData.data(), lightData.size(),
...
bool CUDAMiner::innerInit(size_t numDevices, ethash_light_t _light, uint8_t const* _lightData, uint64_t _lightSize, unsigned _deviceId, bool _cpyToHost, uint8_t* &hostDAG)
{
	try
	{
		if (numDevices == 0)
...
void CUDAMiner::search(uint8_t const* header, uint64_t target, bool _ethStratum, uint64_t _startN)
{
